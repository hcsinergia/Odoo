...
class ApiWsAuthenticate:
  ...
  def get_token(self):
    ...
    if datetime.now().date() > datetime.strptime(expiration, '%Y-%m-%d %H:%M:%S').date() or token == 'TOKENINVALIDO':
    ...
    #session_status = self.check_session(token)
    #if session_status['Erroresnegocio'] == 'Sesión inválida' and token != 'TOKENINVALIDO':
    #  token = 'TOKENINVALIDO'
    #  self.config_parameter.set_param('sudameris.token', token)
    #  self.get_token()
    return token
  ...
  def ws_authenticate(self, *args, **kwargs):
    ...
    try:
      ...
      _logger.debug(['ws_authenticate', response.text])
  ...
  def check_session(self, token):
      from .ws_estado_ca import ApiWsEstadoCuenta
      self.authenticate['Btinreq']['Token'] = token
      _api = ApiWsEstadoCuenta(self.base_url, self.authenticate)
      return _api.ws_estado_ca(1704048, 0, 6900) # Valores por default para checkear si responde bien la consulta